...
JSONAir uses a standard Go toolchain and a `Makefile` for convenience. The end result is five standalone static binaries — the server, the agent, the encryption utility, the admin web interface, and the write-only API.
...
The simplest way to build everything is to run `make` from the repository root. This will tidy the Go module dependencies and compile every binary in one step.
...
bin/
  ...
  jsonair-write/jsonair-write            ← the write-only API
```
...
```bash
go build -o bin/jsonair-write/jsonair-write ./cmd/jsonair-write
```
