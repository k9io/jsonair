...
`jsonair-admin` needs `SELECT, INSERT, UPDATE, DELETE` on `configurations` and `SELECT` on `keys`. `jsonair-write` has its own, narrower grants; see [2.9 The Write API](2.9-write-api.md).
...
Stores the keys used by the optional write-only API (`jsonair-write`). These are completely separate from `keys`: a read key is never accepted by the write API, and a write key is never accepted by the read API. See [2.9 The Write API](2.9-write-api.md).
...
> **Other ways to add configurations:** `jsonair-admin` ([2.8](2.8-admin-web-interface.md)) does the base64 and encryption steps for you from a web form, and `jsonair-write` ([2.9](2.9-write-api.md)) does the same for other services over an API. You only need to run `jsonair-encrypt` and `INSERT` by hand if you are not using either of them.
...
**Add the `write_keys` table** (only if you will run `jsonair-write`): run the `CREATE TABLE IF NOT EXISTS `write_keys`` statement from `sql/jsonair.sql`. It does not touch existing tables.
