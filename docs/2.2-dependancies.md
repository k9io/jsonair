...
JSONAir requires a MySQL-compatible database to store API keys (read and write) and configuration data.
...
| `github.com/goccy/go-yaml` | v1.19.2 | YAML syntax checking (`jsonair-admin` and `jsonair-write`) |
| `github.com/fatih/color` | v1.19.0 | Colorized log output |
| `golang.org/x/time` | v0.15.0 | Token-bucket rate limiter for the auth endpoints |
