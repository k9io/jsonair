...
> **Looking for an API instead of a web page?** Other services can create, update and delete configurations through the separate, write-only `jsonair-write` program. See [2.9 The Write API](2.9-write-api.md).
...
Sessions expire 4 hours after you log in (activity does not extend them). You will be redirected to the login page when your session expires.
...
| **Configuration Data** | Paste the raw configuration text here. Any format is accepted — YAML, JSON, TOML, plain text, binary, etc. The admin interface does not require or check a particular format when you save. |

Click **Create** to save. The raw text is base64-encoded and encrypted before being written to the database.

A configuration is unique per **Key (UUID)**, **Type** and **Name**. Creating a second one with the same three values fails with a database error (duplicate entry); edit the existing record instead.
...
The edit form also has **Verify JSON**, **Verify XML** and **Verify YAML** buttons. They check the syntax of the text in the box and report whether it is valid. They are a convenience only — saving is never blocked by them.
