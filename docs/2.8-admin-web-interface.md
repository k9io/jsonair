...
Navigate to `http://your-host:8080` (or the address set in `HTTP_LISTEN`). You will be redirected to the login page. Enter the `ADMIN_USERNAME` and `ADMIN_PASSWORD` values from your environment.
...
* The session cookie is **HTTP-only** and **SameSite=Strict** to mitigate XSS and CSRF attacks. Consider placing the admin interface behind a TLS-terminating reverse proxy (nginx, Caddy, etc.) if running over an untrusted network.
