---
description: How to encrypt and decrypt configuration data for storage in JSONAir.
---
...
The `jsonair-encrypt` tool handles the encryption step when you insert data into the database by hand. You do not need it if you use `jsonair-admin` ([2.8](2.8-admin-web-interface.md)) or `jsonair-write` ([2.9](2.9-write-api.md)); both base64-encode and encrypt for you, in exactly the same form.
...
| Flag | Behaviour |
|------|-----------|
| _(none)_ | Encrypt: reads plaintext, writes encrypted Base64 |
...
`CONFIG_ENCRYPT_SECRET` must be the **same value** everywhere configuration data is written or read: the `jsonair-encrypt` tool, `jsonair-admin`, `jsonair-write` and the `jsonair` server. If they differ, the server will fail to decrypt and return a `500` error.

**Best practices:**

* Generate a strong secret: `openssl rand -hex 32`
* Keep `CONFIG_ENCRYPT_SECRET` separate from every other secret (`TOKEN_HMAC_SECRET`, `JWT_TOKEN_SECRET`, `ADMIN_SESSION_SECRET`, `WRITE_TOKEN_HMAC_SECRET`, `WRITE_JWT_TOKEN_SECRET`) — use a distinct value for each
...
3. Update `CONFIG_ENCRYPT_SECRET` on the server, `jsonair-admin` and `jsonair-write` (whichever you run) and restart them.
...
Until step 3 is complete, keep the old server running — it cannot read rows encrypted with the new key. Stop `jsonair-admin` and `jsonair-write` for the duration, so nothing is written with the old key while you rotate.
