...
By default, log messages go to the local syslog daemon (and to the console). To send them to a remote syslog server instead:

| Variable | Description | Default |
|----------|-------------|---------|
| `SYSLOG_HOST` | `local` for the local syslog daemon, or the `address:port` of a remote syslog server (e.g. `10.0.0.5:514`) | `local` |
| `SYSLOG_PROTO` | `udp` or `tcp`. Only used when `SYSLOG_HOST` is a remote server. | `udp` |

If syslog cannot be reached, JSONAir logs to the console only. A malformed `SYSLOG_HOST` or an invalid `SYSLOG_PROTO` stops the server at startup.

> **Note:** Each log message opens its own connection to the syslog server. Prefer `udp` (or a local syslog daemon that forwards to your central server); with `tcp`, an unreachable syslog server can slow request handling down.
...
> **Related:** to create, update or delete configurations from another service, see [2.9 The Write API](2.9-write-api.md). It is a separate program with its own settings and secrets.
...
JSONAir exposes an unauthenticated health endpoint at `/health`. It pings the database and returns HTTP `200` when healthy or HTTP `503` when the database is unreachable.
...
This endpoint is intentionally unauthenticated so that load balancers, orchestrators, and container runtimes can poll it without credentials.
